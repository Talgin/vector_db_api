...
class FaceModel:
  def __init__(self, gpu_id, model, ga_model, threshold, det, flip, image_size):
    ...
    self.det = det
    self.det_minsize = 50
    self.det_threshold = [0.6,0.0,0.2]
    #self.det_factor = 0.9
  ...
  def get_input(self, face_img):
    ...
    # tr = cv2.imwrite('/home/ti/Downloads/for_Rus/media/crops/'+str(random.randint(1,10000))+'.jpg', nimg)
    # print(tr)
  ...
  def get_feature(self, aligned):
    ...
    # print('Outputs: ' + str(self.model.get_outputs()))
    embedding = self.model.get_outputs()[0].asnumpy()
    embedding = sklearn.preprocessing.normalize(embedding).flatten()
    return embedding

  def get_features_batch(self, aligned):
    input_blob = np.expand_dims(aligned, axis=0)
