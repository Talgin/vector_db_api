...
@app.post("/faiss/process_folder/", status_code=200)
async def process_folder(response: Response, background_tasks: BackgroundTasks, new_folder: str = Form(...)):
    # if not os.path.exists(os.path.join(settings.ALL_PHOTOS_FOLDER, old_folder)):
    #     response.status_code = status.HTTP_404_NOT_FOUND
    #     return {'status': 'error', 'message': 'No folder '.format(new_folder)}
    # else:
    #     old_path_set = set(os.listdir(os.path.join(settings.ALL_PHOTOS_FOLDER, old_folder)))
    if not os.path.exists(os.path.join(settings.ALL_PHOTOS_FOLDER, new_folder)):
        response.status_code = status.HTTP_404_NOT_FOUND
        return {'status': 'error', 'message': 'No folder {}'.format(new_folder)}
    else:
